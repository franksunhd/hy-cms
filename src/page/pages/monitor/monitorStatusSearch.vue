	...
	<Box>
		...
		<div class="padding20 clearfix">
			...
			<el-table
				...
				<el-table-column width="70px" :label="$t('monitorStatusSearch.columnStatus')" header-align="center" align="center">
					<template slot-scope="scope">
						<el-tooltip v-if="scope.row.status == 99" effect="dark" :content="dictionaryMap.AlarmSeverity[scope.row.status]" placement="top-start">
							<span class="iconfont iconfontError">&#xe64a;</span>
						</el-tooltip>
						<el-tooltip v-if="scope.row.status == 66" effect="dark" :content="dictionaryMap.AlarmSeverity[scope.row.status]" placement="top-start">
							<span class="iconfont iconfontWarn">&#xe649;</span>
						</el-tooltip>
						<el-tooltip v-if="scope.row.status == 22" effect="dark" :content="dictionaryMap.AlarmSeverity[scope.row.status]" placement="top-start">
							<span class="iconfont iconfontDisable">&#xe64f;</span>
						</el-tooltip>
						<el-tooltip v-if="scope.row.status == 11" effect="dark" :content="dictionaryMap.AlarmSeverity[scope.row.status]" placement="top-start">
							<span class="iconfont iconfontDisable">&#xe64e;</span>
						</el-tooltip>
						<el-tooltip v-if="scope.row.status == 33" effect="dark" :content="dictionaryMap.AlarmSeverity[scope.row.status]" placement="top-start">
				...
				<el-table-column prop="roomName" :label="$t('monitorStatusSearch.columnStatusText')" header-align="left" align="left">
					<template slot-scope="scope">
						<el-tooltip effect="dark" placement="left-start">
							<div slot="content" style="width: 150px">
				...
				<el-table-column width="120px" prop="framePosition" :label="$t('monitorStatusSearch.columnMonitorRate')" header-align="center" align="center">
					<template slot-scope="scope">
						<span>{{dictionaryMap.MonitorRate[scope.row.deviceMonitorAttrList[0].monitorInterval]}}</span>
					</template>
				</el-table-column>
				<el-table-column width="140px" prop="ip" :label="$t('monitorStatusSearch.columnLastModifyTime')" header-align="center" align="center">
					<template slot-scope="scope">
		...
		<el-dialog
			...
			<span slot="footer">
				<el-button type="primary" class="alertBtn" @click="dialogVisible_monitorInfo = false">{{$t('public.confirm')}}</el-button>
				<el-button type="default" class="alertBtn" @click="dialogVisible_monitorInfo = false">{{$t('public.cancel')}}</el-button>
			</span>
		</el-dialog>
	</Box>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//行的点击事件
			cellClickColumnTable(row,column,cell,event){
				var _t = this;
				//if (column.label === _t.$t('monitorStatusSearch.columnStatusText')) {
					//解析 result 字段中的 json 串
					var resultConcentArr = JSON.parse(row.result);
					var listArr = new Array();
					for (var key in resultConcentArr) {
						var obj = new Object();
						obj.key = key;
						obj.value = resultConcentArr[key];
						listArr.push(obj);
					}
				...
				_t.$api.post('system/basedata/maplist', {
					...
					switch (res.status) {
						case 200:
							...
							workStatus.unshift({name:'批量操作',value:null});
