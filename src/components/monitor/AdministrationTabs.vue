<template>
	<div class="tabs-Box">
			...
			<el-tab-pane
				...
				<div v-if="isShowTabBox" class="administration-box">
					...
					<el-tabs
						...
						type="card">
						<!--监测详情-->
						<el-tab-pane :label="$t('administrationTabs.monitorDetail')" name="one" v-loading="loadingMonitor">
						...
						<!--事件日志-->
						<el-tab-pane :label="$t('administrationTabs.formEventTitle')" name="six" v-loading="eventLog_loading">
							<el-form :model="eventLogForm" inline style="padding-top: 10px;">
								<!--过滤器-->
								<el-form-item :label="$t('administrationTabs.formEventType') + '：'">
									<el-select clearable v-model="eventLogForm.logType" class="width200">
										<el-option v-for="(item,index) in eventLogTypeList" :key="index" :label="item.label"
															 :value="item.value"/>
									</el-select>
								</el-form-item>
								<!--日志级别-->
								<el-form-item :label="$t('administrationTabs.formEventLevel') + '：'">
									<el-select clearable v-model="eventLogForm.severity" class="width200">
										<el-option v-for="(item,index) in eventLogSeverityList" :key="index" :label="item.label"
															 :value="item.value"/>
									</el-select>
								</el-form-item>
								<!--发生日期-->
								<el-form-item :label="$t('administrationTabs.formEventDateTime') + '：'">
									<el-date-picker
										v-model="eventLogForm.dateTime"
										type="daterange"
										:range-separator="$t('public.to')"
										:start-placeholder="$t('public.startTime')"
										:end-placeholder="$t('public.endTime')"
								...
								<!--日志内容-->
								<el-form-item :label="$t('administrationTabs.formEventDescription') + '：'">
									<el-input @keyup.enter.native="getEventLogData" v-model="eventLogForm.description" class="width200"/>
								</el-form-item>
								<el-form-item>
									<el-button type="primary" @click="getEventLogData" class="queryBtn">{{$t('public.query')}}</el-button>
								</el-form-item>
							</el-form>
							<el-table :data="eventLogList" border stripe>
								<el-table-column width="230px" :label="$t('administrationTabs.columnEventLogLevel')">
									<template slot-scope="scope">
										<span v-if="scope.row.severity === 'Critical'">{{$t('administrationTabs.formEventLogSeverityCritical')}}</span>
										<span v-if="scope.row.severity === 'Ok'">{{$t('administrationTabs.formEventLogSeverityOk')}}</span>
									</template>
								</el-table-column>
								<el-table-column width="230px" :label="$t('administrationTabs.columnEventLogType')">
									<template slot-scope="scope">
										<span v-if="scope.row.logType === 1">{{$t('administrationTabs.formEventLogTypeSystem')}}</span>
										<span v-if="scope.row.logType === 2">{{$t('administrationTabs.formEventLogTypeLifecycle')}}</span>
										<span v-if="scope.row.logType === 3">{{$t('administrationTabs.formEventLogTypeSyslog')}}</span>
										<span v-if="scope.row.logType === 4">{{$t('administrationTabs.formEventLogTypeSnmpTrap')}}</span>
									</template>
								</el-table-column>
								<el-table-column width="230px" :label="$t('administrationTabs.columnEventLogDateTime')">
									<template slot-scope="scope">
										<span>{{scope.row.lastModifyTime | dateFilter}}</span>
									</template>
								</el-table-column>
								<el-table-column prop="description"
																 :label="$t('administrationTabs.columnEventLogDescription')"></el-table-column>
							</el-table>
							<pages
								:total='optionsEventLog.total'
								:currentPage='optionsEventLog.currentPage'
								:page-size="optionsEventLog.pageSize"
								@handleSizeChangeSub="handleSizeChangeEventSub"
								@handleCurrentChangeSub="handleCurrentChangeEvent"/>
						</el-tab-pane>
						<!--位置信息-->
						<!--
						<el-tab-pane :label="$t('administrationTabs.locationInformation')" name="seven">
						...
						<!--维保信息-->
						<!--
						<el-tab-pane :label="$t('administrationTabs.maintenanceInformation')" name="eight">
						...
						<!--变更信息-->
						<!--
						<el-tab-pane :label="$t('administrationTabs.changeInformation')" name="nine">
		...
		<el-dialog
			...
			:close-on-click-modal="false"
			:close-on-press-escape="false">
			<div v-loading="thresholdContent_loading">
				<!--设备信息详情-->
			...
			<div slot="footer">
				<!--确认按钮-->
				<el-button type="primary" class="alertBtn" @click="addMonitor">{{$t('public.save')}}</el-button>
...
<script>
	...
	import {dateFilter, dateFilterDay, dateFilterChangeDay} from "../../assets/js/filters";
	...
	export default {
		...
		data() {
			return {
				...
				loadingMonitor: false, // 监测详情 loading
				// 告警事件模块
				...
				thresholdContent_loading: false, // 阈值弹出层
				// 事件日志
				eventLog_loading: false, // 事件日志页签 拼命加载中
				// 事件日志查询表单
				eventLogForm: {
					logType: '', // 日志类型
					severity: 'all', // 日志级别
				...
				// 日志类型下拉框
				eventLogTypeList: [
					{label: this.$t('administrationTabs.formEventLogTypeSystem'), value: 1},
					{label: this.$t('administrationTabs.formEventLogTypeLifecycle'), value: 2},
					{label: this.$t('administrationTabs.formEventLogTypeSyslog'), value: 3},
					{label: this.$t('administrationTabs.formEventLogTypeSnmpTrap'), value: 4},
				],
				// 日志级别下拉框
				eventLogSeverityList: [
					{label: this.$t('administrationTabs.formEventLogSeverityAll'), value: 'all'},
					{label: this.$t('administrationTabs.formEventLogSeverityCritical'), value: 'Critical'},
					{label: this.$t('administrationTabs.formEventLogSeverityOk'), value: 'Ok'},
				],
				// 事件日志类别分页
				optionsEventLog: {
					total: 0, // 总条数
		...
		methods: {
			...
			clickTabsTopTitle(tab) {
				...
				if (tab.name === 'one') {
					...
					_t.activeNameTabs = 'six';
					// 获取事件日志 接口数据
					_t.getEventLogData(false);
					// 清除定时器
					_t.clearIntervalFun();
				}
			},
			...
			getMonitorThreshold(row) {
				...
				// 阈值弹出层加载
				_t.thresholdContent_loading = true;
			...
			getMonitorSelectData(val) {
				...
				_t.$api.get('monitor/deviceMonitor/all', {
					...
					switch (res.status) {
						case 200:
							// 阈值弹出层加载
							_t.thresholdContent_loading = false;
				...
				treeData.forEach((item) => {
					...
					if (item.nodeType === 1) {
						...
						if (item[valField] == val) {
							...
							if (monitorLevelFirst.deviceMonitorList && monitorLevelFirst.deviceMonitorList !== null) {
			...
			switchMonitoringDetailsData(treeData) {
				...
				treeData.forEach((item) => {
					...
					if (item.nodeType === 0) {
						// 子集循环 (二级)
						item.deviceMonitorListArrStart = item.deviceMonitorListArrStart === null ? [] : item.deviceMonitorListArrStart;
			...
			// 事件日志 => 获取事件日志 接口数据
			getEventLogData(flag) {
				var _t = this;
				_t.eventLog_loading = true;
				// flag 刚进来的时候重置
				if (flag === false) {
					_t.resetEventLogData();
				}
				// 处理发生 日期
				var eventLogFormTime = _t.eventLogForm.dateTime;
				var dateTime = [];
				if (eventLogFormTime !== null) {
					dateTime[0] = dateFilterDay(eventLogFormTime[0]);
					dateTime[1] = dateFilterDay(eventLogFormTime[1]);
					eventLogFormTime = dateTime.join(',');
				} else {
					eventLogFormTime = null;
				}
				_t.$api.get('log/monitorLog/pagelist', {
					jsonString: JSON.stringify({
						monitorLog: {
							deviceId: _t.activeDeviceId,
							logType: _t.eventLogForm.logType === '' ? null : _t.eventLogForm.logType,
							severity: _t.eventLogForm.severity === '' ? null : (_t.eventLogForm.severity === 'all' ? null : _t.eventLogForm.severity),
							dateTime: eventLogFormTime,
							description: _t.eventLogForm.description.trim() === '' ? null : _t.eventLogForm.description.trim()
						},
						currentPage: _t.optionsEventLog.currentPage,
						pageSize: _t.optionsEventLog.pageSize
					})
				}, function (res) {
					_t.eventLog_loading = false;
					switch (res.status) {
						case 200:
							if (res.data && res.data !== null) {
								if (res.data.list && res.data.list !== null) {
									_t.eventLogList = res.data.list;
									_t.optionsEventLog.currentPage = res.data.currentPage;
									_t.optionsEventLog.total = res.data.count;
								} else {
									_t.eventLogList = [];
									_t.optionsEventLog.currentPage = 1;
									_t.optionsEventLog.total = 0;
								}
							}
							break;
						default:
			...
			resetEventLogData() {
				var _t = this;
				_t.eventLogForm.logType = ''; // 日志类型
				_t.eventLogForm.severity = 'all'; // 日志级别
				_t.eventLogForm.dateTime = new Array();
				// 设置 事件日志 发生时间 的默认值
				var beginTime = new Date();
				var endTime = dateFilterChangeDay(new Date().getTime(), 1);
				_t.eventLogForm.dateTime[0] = beginTime;
				_t.eventLogForm.dateTime[1] = new Date(endTime);
				_t.eventLogForm.description = ''; // 日志内容
				_t.optionsEventLog.currentPage = 1;
				_t.optionsEventLog.total = 0;
			},
			// 事件日志 => 改变当前页码
			handleCurrentChangeEvent(val) {
				var _t = this;
				_t.optionsEventLog.currentPage = val;
				_t.getEventLogData();
			},
			// 事件日志 => 改变每页显示条数
			handleSizeChangeEventSub(val) {
				var _t = this;
				_t.optionsEventLog.pageSize = val;
				_t.getEventLogData();
			},
			// 字典 => 获取状态字典 map
			getStatusDataBase() {
				...
				_t.$api.post('system/basedata/map', {
					...
					switch (res.status) {
						case 200:
							...
							// 给告警事件中告警级别下拉框加数据
							var labelValue = _t.$t('public.all');
		...
		created() {
			var _t = this;
			if (localStorage.getItem('hy-language') !== 'null' && localStorage.getItem('hy-language') !== null) {
				// 加载数据字典 map
