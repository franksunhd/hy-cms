	...
	<div>
		...
		<el-dialog
			...
			<el-table
				...
				indent
				style="width: 100%">
				<el-table-column :label="$t('EquipmentMonitoring.TheLineNumbers')"
				...
				<el-table-column :label="$t('EquipmentMonitoring.TheThird')" align="center">
					<template slot-scope="scope">
						<el-input size="small" v-model="scope.row.amount2"
											:placeholder="$t('EquipmentMonitoring.PleaseEnterContent')"></el-input>
					</template>
				</el-table-column>
		...
		<el-dialog class="EquipmentMonitoringBj" :close-on-click-modal="false"
			...
			<el-form :model="formItem" inline label-width="150px" :rules="rules" ref="roleName">
				...
				<el-form-item class="star" :label="$t('EquipmentMonitoring.TheNameOfTheNode') + '：'"
											prop="catalogName">
			...
			<div slot="footer" class="dialog-footer">
				...
				<el-button @click="getEditDeviceQx">{{$t('EquipmentMonitoring.cancel')}}
				</el-button>
			</div>
		</el-dialog>
		<!--删除设备分组-->
		<el-dialog class="EquipmentMonitoringSc"
			...
			<span slot="footer" class="dialog-footer">
							...
							<el-button
								@click="getRemoveQx">{{$t('EquipmentMonitoring.cancel')}}</el-button>
						</span>
		</el-dialog>
	</div>
</template>
