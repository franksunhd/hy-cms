	...
	<div class="transfer" :style="{ width, height }">
		...
		<template v-else-if="mode == 'addressList'">
          ...
          <span>{{ fromTitle }}</span>
            ...
            <img class="move_up_img move_down_img" v-else src="../assets/img/shang.png" alt="" @click="moveUp('down')"/>
          </h3>
					<!-- 内容区 -->
          ...
          <div class="transfer-main" v-if="move_up">
            ...
            <ul class="address-list-ul">
              <li class="address-list-li"
                ...
                :key="item[node_key]">
                <label>{{ item[defaultProps.label] }}{{ item.Email }}</label>
                <i class="address-list-del"
                  @click="clearList(2, item[node_key])">x</i>
              </li>
            </ul>
          </div>
        </div>
      ...
      switch (type) {
        ...
        case 2:
          this.secret_receiver =
            id == "all"? []: this.secret_receiver.filter(item => item[this.node_key] != id);
          break;
      }
      // 传递信息给父组件
